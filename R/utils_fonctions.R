...
twitServer <- function(id,list_var) {
  moduleServer(
    ...
    function(input, output, session,list_var) {
      ns <- session$ns
      ...
      output$choix_seuils <- renderUI({
        if(un_seul_seuil=="Non"){
          
          tagList(
            lapply(1:length(nom_var_quali), function(i) {
              
              list(
                numericInput(inputId =ns(paste( "seuil_", x, sep = "")), label= x , value = 0 )
      ...
      return(reactive({list( input$theta_P_min, 
                   ...
                   "valeur_seuil" = ifelse_perso(is.null(input$valeur_seuil),sapply(nom_var_quali,input[[paste( "seuil_", x, sep = "")]]),input$valeur_seuil)

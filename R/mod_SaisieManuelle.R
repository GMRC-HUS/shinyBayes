...
mod_SaisieManuelle_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$KappaMAIN <- renderTable(
      {
        ...
        if (all(Mat2[, 1] == Mat2[, 2])) {
          RESULTAT <- c(1, 1, 1)
        } else {
          lkappa.boot <- function(data, x) {
            suppressWarnings(kappa2(data[x, ]))$value
          }
    ...
    output$mytableCONCORDANCE2 <- renderTable(
      {
        if (is.null(input$Concoman1)) {
          return()
        }
        if (input$Concoman1 == "" | !length(strsplit(input$Concoman1, " ")[[1]]) == length(strsplit(input$Concoman2, " ")[[1]])) {
          return()
        }
    ...
    output$ConcordanceManuelleINTERV2 <- renderPrint({
      ...
      if (!length(strsplit(input$Concoman1, " ")[[1]]) == length(strsplit(input$Concoman2, " ")[[1]])) {
        return("Les variables doivent avoir la même longueur")
      }
      ...
      cat(
        "Le coefficient de concordance Kappa de Cohen est estimé à", RESULTAT[1],
    ...
    output$LandisEtKoch <- renderTable(
      {
        data.frame(
          Kappa = c("0-0.2", "0.21-0.40", "0.41-0.60", "0.61-0.80", "0.81-1"),

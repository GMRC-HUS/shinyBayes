  ...
  if (situation == 1) {
    ...
    f <- function(t) {
      integrate(function(p1) {
        sapply(p1, function(p1) {
          p1^(a1 - 1) * (1 - p1)^(b1 - 1) / beta(a1, b1) * integrate(function(p2) p2^(a2 - 1) * (1 - p2)^(b2 - 1) / beta(a2, b2), 0, t + p1)$value
        })
      }, -t, 1)$value
    }
    g <- function(t) {
      1 - integrate(function(p1) {
        sapply(p1, function(p1) {
          p1^(a1 - 1) * (1 - p1)^(b1 - 1) / beta(a1, b1) * (1 - integrate(function(p2) p2^(a2 - 1) * (1 - p2)^(b2 - 1) / beta(a2, b2), 0, t + p1)$value)
        })
      }, 0, 1 - t)$value
    }
...
logistique.variable.cont <- function(y, x, ic = FALSE, titre = "") {
  ...
  if (nlevels(as.factor(x)) == 2) {
    situation <- 0
    print("Variable binaire et non quantitative")
  }
  if (nlevels(as.factor(y)) != 2) {
    situation <- 0
    print("Variable réponse non binaire")
  }
  ...
  if (situation == 99) {
    ...
    FP <- T[2, 1]
    FN <- T[1, 2]
    V1 <- cbind(VP, VN, FP, FN)
    colnames(V1) <- c("VP", "VN", "FP", "FN")

    VPP <- round(VP / (VP + FP), 2)
    VPN <- round(VN / (VN + FN), 2)
    Exact <- round((VP + VN) / (VP + VN + FP + FN), 2)
    Erreur <- round((FP + FN) / (VP + VN + FP + FN), 2)
    V2 <- cbind(VPP, VPN, Exact, Erreur)
    colnames(V2) <- c("VPP", "VPN", "Exactitude", "Taux d'erreur")
...
logit.ss.latex <- function(y, x, ic = FALSE, titreSL = "") {
  ...
  if (all(levels(as.factor(as.numeric(y))) == c("0", "1"))) {
    y2 <- y
  }
  if (all(levels(as.factor(as.numeric(y))) == c("1", "2"))) {
    y2 <- as.numeric(y) - 1
  }
  ...
  if (cas == 1) {
    return(logistique.variable.bin(y2, x))
  }
  if (cas == 2) {
    return(logistique.variable.cont(y2, x, ic, titre = titreSL))
  }
}
...
logist <- function(y, x, ic = FALSE, latex = FALSE, titre = 0) {
  ...
  if (titre == 0) {
    MainT <- paste("Courbe ROC :", deparse(substitute(y)), "~", deparse(substitute(x)))
  } else {
    MainT <- titre
  }

  if (latex == FALSE) {
    return(logit.ss.latex(y, x, ic, titreSL = MainT))
  }
  if (latex == TRUE) {
    L <- logit.ss.latex(y, x, ic, titreSL = MainT)
    print(L)
...
croisements <- function(numero.variable.dinteret, D, qualiF, quantiF, affichage = 40) {
  ...
  {
    if (is.factor(D[, i]) == TRUE) {
      Factor <- c(Factor, i)
    }
    if (is.numeric(D[, i]) == TRUE) {
      VarNumeric <- c(VarNumeric, i)
    }
  }
  ...
  if (sum(Factor, na.rm = TRUE) != 0) {
    for (i in 1:(length(Factor))) {
      if (nlevels(D[, Factor][, i]) <= mod.max) {
        Factinfseuil <- c(Factinfseuil, Factor[i])
      }
      if (nlevels(D[, Factor][, i]) > mod.max) {
        Factsupseuil <- c(Factsupseuil, Factor[i])
      }
    }
  ...
  for (i in 1:NbNumeric) {
    if (nlevels(as.factor(D[, VarNumeric][, i])) > mod.max) {
      quanti <- c(quanti, VarNumeric[i])
    }
    if (nlevels(as.factor(D[, VarNumeric][, i])) <= mod.max) {
      quali <- c(quali, VarNumeric[i])
    }
  }
  ...
  if (situation < 5) {
    if (missing(quantiF) & missing(qualiF)) {
      choix <- 2
      situation <- 1
    } else {
      if (missing(quantiF)) {
        choix <- 1
        qualitativesM <- qualiF
        quantitativesM <- -1
        situation <- 2
      } else {
        if (missing(qualiF)) {
          choix <- 1
          quantitativesM <- quantiF
          qualitativesM <- -1
          situation <- 3
        } else {
          choix <- 1
    ...
    if (choix == 1) {
      qualitatives <- qualitativesM
      quantitatives <- quantitativesM
    } else {
      if (choix == 2) {
        qualitatives <- quali
        quantitatives <- quanti
      } else {
        stop("Erreur de choix")
      }
    }

    autorisation <- rep(0, (length(Factinfseuil)))
    for (i in 1:(length(Factinfseuil))) {
      if ((is.na(Factinfseuil[i]) == 0) & (Factinfseuil[i] != 0)) {
        autorisation[i] <- 1
      }
    }
    if (situation == 1) {
      if ((sum(autorisation)) == (length(autorisation))) {
        qualitatives <- c(qualitatives, Factinfseuil)
      }
    }
  }
  ...
  if (situation == 1) {
    Affichage.Nombre.variables.quali <- (length(qualitatives) - 1)
    Affichage.Nombre.variables.quanti <- length(quantitatives)
  }
  if (situation == 2) {
    Affichage.Nombre.variables.quali <- (length(qualiF))
    Affichage.Nombre.variables.quanti <- 0
  }
  if (situation == 3) {
    Affichage.Nombre.variables.quali <- 0
    Affichage.Nombre.variables.quanti <- length(quantiF)
  }
  if (situation == 4) {
    Affichage.Nombre.variables.quali <- (length(qualiF))
    Affichage.Nombre.variables.quanti <- length(quantiF)
  ...
  if (situation == 5) {
    Affichage.Nombre.variables.quali <- 0
    Affichage.Nombre.variables.quanti <- 1
  }
  if (situation == 6) {
    Affichage.Nombre.variables.quali <- 1
    Affichage.Nombre.variables.quanti <- 0
  }
  ...
  if (tableaux.croises == 1) {
    if ((situation == 1) | (situation == 2) | (situation == 4) | (situation == 6)) {
      ...
      cat(noquote("|                                                            ||\n"))
      cat(noquote("|                                                            ||\n"))
      if (Affichage.Nombre.variables.quali > 9) {
        Affichage.espace.quali <- "               ||"
      } else {
        Affichage.espace.quali <- "              ||"
      }
      ...
      for (i in 1:(length(qualitatives))) {
        if (colnames(D[, qualitatives])[i] == colnames(D)[ny]) {
          numero <- i
        } else {
          numero <- 0
        }
      ...
      for (i in 1:(length(qualitatives))) {
        if (i != numero) {
          ...
          if ((LTABLE == 1) & (is.na(D[, qualitatives][, i][1]) == 1)) {
            VIDE <- 1
          } else {
            VIDE <- 0
          }
          if (VIDE == 0) {
            ...
            Alig <- A
            for (k in 1:Nblignes) {
              Alig[k, ] <- round(A[k, ] / sum(A[k, ]), 3)
            } ### Matrice des effectifs en ligne
            colnames(Alig) <- c(paste(attr(table(D[, qualitatives][i]), "dimnames")[[1]], "%lign"))
            Acol <- A
            for (m in 1:Nbcolonnes) {
              Acol[, m] <- round(A[, m] / sum(A[, m]), 3)
            } ### Matrice des pourcentages en colonne
            colnames(Acol) <- c(paste(attr(table(D[, qualitatives][i]), "dimnames")[[1]], "%col"))
            Atot <- round(A / sum(A), 3)
            colnames(Atot) <- c(paste(attr(table(D[, qualitatives][i]), "dimnames")[[1]], "%tot")) ### Matrice des pourcentages totaux
            ...
            if (length(A) > 4) {
              Acomp <- list(Tableau = A, Pourcentages.lignes = Alig, Pourcentages.colonnes = Acol, Pourcentages.total = Atot)
            } ### Condition pour voir si c'est pas trop long
            ### Sinon reprendre dans une autre ligne
            p <- chisq.test(A)$p.value
            p2 <- try(fisher.test(A)$p.value, silent = TRUE)
            ...
            if (length(A) == 4) {
              ...
              ORCI <- round(ORCI, digits = 3)
              conjonction <- "dans ["
            }
            if (length(A) != 4) {
              OR <- "Non_calculable"
              ORCI <- c("", "")
              conjonction <- "Problème_Taille"
            ...
            if (p < 0.05) {
              ...
              p <- round(p, 2)
            } ### Affichage des etoiles si le test est signif
            if (is.numeric(p2)) {
              if (p2 < 0.05) {
                p2 <- paste(round(p2, 2), "           ***")
              } else {
            ...
            if (choix.de.sortie.R.1.LaTeX.2 == 1) {
              print(Acomp) ### Affichage des resultats des tests
            ...
            if (choix.de.sortie.R.1.LaTeX.2 == 2) {
              print(xtable(A, caption = paste("Chi2", round(p, 3), "/", "Fisher", try(round(p2, 3), silent = TRUE))))
            }
          } else {
    ...
    if (situation == 5) {
      quantitatives <- c(quantitatives, quantitatives)
    }
    ...
    if ((situation == 1) | (situation == 3) | (situation == 4) | (situation == 5)) {
      ...
      cat(noquote("|                                                            ||\n"))
      cat(noquote("|                                                            ||\n"))
      if (Affichage.Nombre.variables.quanti > 9) {
        Affichage.espace <- "              ||"
      } else {
        Affichage.espace <- "             ||"
      }
      ...
      for (i in 1:borne)
      {
        ...
        if (((LT2 == 1) & (is.na(D[, quantitatives][, i][Y == attr(table(Y), "dimnames")$Y[2]][1] == TRUE))) | ((LT1 == 1) & (is.na(D[, quantitatives][, i][Y == attr(table(Y), "dimnames")$Y[1]][1] == TRUE)))) {
          ...
          if ((p.norm.1 > 0.05) & (p.norm.2 > 0.05)) {
            normalite <- "Acceptee 2/2"
          }
          if ((p.norm.1 > 0.05) & (p.norm.2 < 0.05)) {
            normalite <- "Rejetee 1/2"
          }
          if ((p.norm.1 < 0.05) & (p.norm.2 > 0.05)) {
            normalite <- "Rejetee 1/2"
          }
          if ((p.norm.1 < 0.05) & (p.norm.2 < 0.05)) {
            normalite <- "Rejetee 0/2"
          }
        }
        if (p < 0.05) {
          p <- paste(round(p, 2), "      ***")
        ...
        if ((p != "MANQUANTS") & (p2 != "MANQUANTS")) {
          V0 <- noquote(cbind(nam[i], "         ", normalite))
        } # Affichage des resultats
        if ((p != "MANQUANTS") & (p2 != "MANQUANTS")) {
          V <- noquote(rbind(paste("Egalite des moyennes : Test de Student    ", p), paste("Comparaison distributions: Test de MW.Wilcoxon", p2)))
        } else {
          V <- cbind("Prob de manquants", p, "Prob de manquants", p2)
        } # Affichage des resultats
        if ((p != "MANQUANTS") & (p2 != "MANQUANTS")) {
          V2 <- noquote(rbind(paste("Egalite des variances: Test de Fisher     ", p3), paste("Egalite des variances: Test de Ansari     ", p4)))
        }
        ...
        if (choix.de.sortie.R.1.LaTeX.2 == 2) {
          print(xtable(A, caption = paste(nam[i], "/", "t.test", round(p, 2), "/", "Wilcoxon", round(p2, 2))))
        }
      }
    }
  }
...
HosLem <- function(modele) {
  ...
  for (k in 1:L) # Vecteur de classes
  {
    bo <- bornes[k + 1]
    for (j in 1:(length(p))) {
      if ((p[j] <= bornes[k + 1]) & (p[j] > bornes[k])) {
        classes[j] <- k + 1
      }
    }
  }
...
desc <- function(D, modmax = 7) {
  if (is.data.frame(D)) {
    cas <- 1
    desc.table(D, modmax = modmax)
  } # DATA.FRAME
  if (is.numeric(D)) {
    cas <- 2
    r <- desc1(D, modmax = modmax)
    print(r)
  } # VARIABLE NUMERIQUE ( Quali ou Quanti )
  if (is.factor(D)) {
    cas <- 3
    r <- desc1(D, modmax = modmax)
    print(r)
  ...
  if (all(cas != c(1, 2, 3))) {
    print("Variable inconnue_ Verifiez la source")
  }
}
...
compare <- function(x, y, paired, modmax) {
  ...
  if (is.factor(x) & is.numeric(y)) {
    if (nlevels(as.factor(y)) <= mod.max) {
      cas <- 1
    } else {
      cas <- 2
    }
  ...
  if (is.numeric(x) & is.factor(y)) {
    if (nlevels(as.factor(x)) <= mod.max) {
      cas <- 1
    } else {
      cas <- 2
    }
  ...
  if (is.numeric(x) & is.numeric(y)) {
    if (nlevels(as.factor(x)) <= mod.max & nlevels(as.factor(y)) <= mod.max) {
      cas <- 1
    }
  }
  if (is.numeric(x) & is.numeric(y)) {
  ...
  if (is.character(x) & is.factor(y)) {
    cas <- 1
  }
  if (is.factor(x) & is.character(y)) {
    cas <- 1
  }


  if (cas == 2) {
    result <- "Erreur: Les variables cont de nature differentes, impossibilite de comparer"
  }
  ...
  if (cas == 3) {
    ...
    if (appa == "FALSE") {
      T[1, 1] <- round(try(t.test(x, y)$p.value), 2)
      T[1, 2] <- round(try(wilcox.test(x, y)$p.value), 2)
    ...
    if (appa == "TRUE") {
      T[1, 1] <- round(try(t.test(x, y, paired = TRUE)$p.value), 2)
      T[1, 2] <- round(try(wilcox.test(x, y, paired = TRUE)$p.value), 2)
  ...
  if (cas == 1) {
    x <- as.factor(x)
    y <- as.factor(y)
    if (length(intersect(levels(x), levels(y))) == 0) {
      result <- "Erreur : Les variables demandees n'ont aucune modalite en commun."
    } else {
      cas <- 3
      ...
      tablx <- table(x)
      pourx <- round(table(x) / length(x[is.na(x) == F]), 3)
      for (i in 1:length(max.lev)) {
        try(Tx[i, 1] <- tablx[names(tablx) == max.lev[i]], silent = TRUE)
      }
      for (i in 1:length(max.lev)) {
        try(Tx[i, 2] <- pourx[names(tablx) == max.lev[i]], silent = TRUE)
      }
      tably <- table(y)
      poury <- round(table(y) / length(y[is.na(y) == F]), 3)
      for (i in 1:length(max.lev)) {
        try(Ty[i, 1] <- tably[names(tably) == max.lev[i]], silent = TRUE)
      }
      for (i in 1:length(max.lev)) {
        try(Ty[i, 2] <- poury[names(tably) == max.lev[i]], silent = TRUE)
      }
  ...
  if ((cas != 1) & (cas != 2) & (cas != 3)) {
    result <- "Erreur: Le type de variable n'est pas reconnu, merci de votre comprehension"
  }
  print(result)
} # fin fonction
...
norm1 <- function(X, alpha = 0.05, precision = 5, aff = 1, graph = 1) {
  ...
  if (all(A >= 0)) {
    sq <- round(shapiro.test(sqrt(A))$p.value, precision)
  } else {
    sq <- "Valeurs negatives ou nulles"
  } # Transfo RACINE
  if (all(A > 0)) {
    lg <- round(shapiro.test(log(A))$p.value, precision)
  } else {
    lg <- "Valeurs negatives ou nulles"
  } # Transfor LOG
  if (all(A >= 0)) {
    ar <- round(shapiro.test(asin(sqrt(A) / 100))$p.value, precision)
  } else {
    ar <- "Valeurs negatives ou nulles"
  } # ARCSIN RACINE

  library(car)
  if (all(A > 0)) {
    Lambda <- powerTransform(A)$lambda
    if (Lambda != 0) {
      A.p <- (A^Lambda - 1) / Lambda
    } else {
      A.p <- log(A)
    }
    bc <- round(shapiro.test(A.p)$p.value, precision)
  } else {
    bc <- "Valeurs negatives ou nulles"
    Lambda <- NA
  }
  ...
  for (i in 1:length(RES[, 1])) {
    if (RES[i, 1] == "Valeurs negatives" | RES[i, 1] == "Valeurs negatives ou nulles") {
      RES[i, 2] <- "NA"
    }
  }
...
norm <- function(x1, x2 = NULL, x3 = NULL, x4 = NULL, x5 = NULL, x6 = NULL, x7 = NULL, x8 = NULL, x9 = NULL, x10 = NULL, x11 = NULL, x12 = NULL, x13 = NULL, x14 = NULL, x15 = NULL) {
  ...
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & is.null(x4)) {
    taille <- 3
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & is.null(x5)) {
    taille <- 4
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & is.null(x6)) {
    taille <- 5
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & is.null(x7)) {
    taille <- 6
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & is.null(x8)) {
    taille <- 7
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & is.null(x9)) {
    taille <- 8
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & is.null(x10)) {
    taille <- 9
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & is.null(x11)) {
    taille <- 10
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & !is.null(x11) & is.null(x12)) {
    taille <- 11
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & !is.null(x11) & !is.null(x12) & is.null(x13)) {
    taille <- 12
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & !is.null(x11) & !is.null(x12) & !is.null(x13) & is.null(x14)) {
    taille <- 13
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & !is.null(x11) & !is.null(x12) & !is.null(x13) & !is.null(x14) & is.null(x15)) {
    taille <- 14
  }
  if (!is.null(x1) & !is.null(x2) & !is.null(x3) & !is.null(x4) & !is.null(x5) & !is.null(x6) & !is.null(x7) & !is.null(x8) & !is.null(x9) & !is.null(x10) & !is.null(x11) & !is.null(x12) & !is.null(x13) & !is.null(x14) & !is.null(x15)) {
    taille <- 15
  ...
  if (taille > 1) {
    ...
    library(car)
    if (all(complet > 0)) {
      Lambda <- round(powerTransform(complet)$lambda, 2)
    }
    Ncol <- dim(x)[2]
    ...
    if (Ncol <= 4) {
      ...
      hist(complet)
      qqnorm(complet)
      qqline(complet, col = "red")
      for (i in 1:(Ncol)) {
        hist(MV[, i][!is.na(MV[, i])], main = paste("Hist de vecteur", i))
...
extreme <- function(x, nlab = 2, labs = as.character(1:length(x)), ylab = "Sorted Data", ...) {
  if (!is.numeric(x)) {
    print("Erreur: le vecteur n'est pas numerique")
  } else {
    ######### 	Creation du graphe	######################
    par(mfrow = c(1, 2))
    ...
    plot(ui, x[i],
      main = "Half-normal plot", xlab = "Half-normal quantiles", ylab = ylab,
...
ggsurv <- function(s, CI = "def", plot.cens = T, surv.col = "gg.def", cens.col = "red", lty.est = 1, lty.ci = 2, cens.shape = 3, back.white = F, xlab = "Time", ylab = "Survival", main = "") {
  ...
  ggsurv.s <- function(s, CI = "def", plot.cens = T, surv.col = "gg.def",
    ...
    pl <- if (plot.cens == T & length(dat.cens) > 0) {
      pl + geom_point(
        data = dat.cens, aes(y = surv), shape = cens.shape,
        col = cens.col
      )
...
correl <- function(x, y, droite = 1, nomx = NULL, nomy = NULL) {
  ...
  if (length(x) == length(y)) {
    r <- rbind(
      c(round(cor.test(x, y, method = "kendall")$estimate, 3), cor.test(x, y, method = "kendall")$p.value),
...
plot.evol2 <- function(DT, groups, main, etyp = 0) {
  ...
  if (par.groupes != 1) {
    ...
    moyenne <- rep(NA, Nt)
    for (m in 1:Nt) {
      moyenne[m] <- mean(DT[, m], na.rm = TRUE)
    }
    for (i in 1:N) {
      v <- DT[i, ]
      points(v, col = i)
  ...
  if (par.groupes == 1) {
    ...
    for (c in 1:Nt) {
      M[, c] <- tapply(DT[, c], groups, mean, na.rm = TRUE)
    }
    for (i in 1:Ngroupes) {
      v <- M[i, ]
      points(v, col = i + 1)
      lines(v, col = i + 1, lwd = 4)
    }
    legend("topleft", as.character(levels(as.factor(groups))),
      col = 1:Ngroupes + 1,
  ...
  if (par.groupes == 1 & etyp == 1) {
    ...
    if (length(Niveaux) > 3) {
      print("L'option etyp n'est pas utilisable pour plus de 3 groupes, au risque d'avoir un graphique trop charge")
    }
  }
}
...
plot.evol <- function(Trajectoires, Groupe = NULL, IC = FALSE, Moyenne = TRUE, Temps = NULL, label_x = "Temps", label_y = "Valeur", labels_ticks_x = NULL) {
  ...
  if (is.null(labels_ticks_x)) {
    labels_ticks_x <- as.character(Temps)
  }
...
burt <- function(x) {
  ...
  for (j in 1:nbre.colonnes) {
    MatRES[, j] <- ifelse(as.factor(x) == levels(as.factor(x))[j], 1, 0)
  }
...
desqlb <- function(X, alpha = rep(1, nlevels(X)), beta = rep(1, nlevels(X)), ic = 0.95) {
  ...
  for (i in 1:nbl) {
    aze[i, 3] <- 100 * (aze[i, 1] + alpha[i]) / (sum(tcr) + alpha[i] + beta[i])
  }
  aze[nbl + 1, 1] <- sum(tcr)
  aze[nbl + 1, 2] <- 100
  nbmq <- table(X, exclude = NULL)[nbl + 1]
  ...
  aze[nbl + 2, 2] <- 100 - aze[nbl + 3, 2]
  for (i in 1:nbl) {
    aze[i, 4] <- 100 * qbeta((1 - ic) / 2, aze[i, 1] + alpha[i], sum(tcr) - aze[i, 1] + beta[i])
  }
  for (i in 1:nbl) {
    aze[i, 5] <- 100 * qbeta(1 - (1 - ic) / 2, aze[i, 1] + alpha[i], sum(tcr) - aze[i, 1] + beta[i])
  }
  return(aze)
}
...
ggboxplot <- function(x, y, Groupe = NULL) {
  ...
  nomy <- deparse(substitute(y))
  if (is.null(Groupe)) {
    DDD <- data.frame(cbind(x, y))
  } else {
    DDD <- data.frame(cbind(x, y, Groupe))
  }
  DDD <- DDD[complete.cases(DDD), ]
  library(ggplot2)
  if (is.null(Groupe)) {
    p <- ggplot(DDD, aes(factor(y), x)) +
      xlab(nomy) +
      ylab(nomx)
  } else {
    p <- ggplot(DDD, aes(factor(y), x, fill = factor(Groupe))) +
      xlab(nomy) +
      ylab(nomx)
  }
...
ggsurvie <- function(x, y, groups = 0, latex = 0, titre = 0) {
  ...
  if (titre == 1) {
    titre.ss.groupe <- paste("Courbe de survie: Variable", nom.y)
  }
  if (groups != 0) {
    nom.groupes <- deparse(substitute(groups))
    if (titre == 1) {
      titre.ac.groupe <- paste("Courbe de survie: Variable", nom.y, "en fonction de:", nom.groupes)
    }
  }

  if (titre == 0) {
    titre.ss.groupe <- titre.ac.groupe <- ""
  }
  if (titre != 0 & titre != 1) {
    titre.ss.groupe <- titre.ac.groupe <- titre
  }
